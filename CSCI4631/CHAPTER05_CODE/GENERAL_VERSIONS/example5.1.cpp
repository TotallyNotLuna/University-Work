...
GLfloat top       =  2.0;
GLfloat bottom    = -2.0;
GLfloat near_val  = -4.0; // had to rename near and far to near_val and far_val
GLfloat far_val   =  4.0; // do get around a name clash in a #define compiling
...
void display()
{
     ...
     projection = Ortho(left, right, bottom, top, near_val, far_val);
...
void mykey(unsigned char key, int mousex, int mousey)
{
   ...
   if(key == 'z') {near_val  *= 1.1; far_val *= 1.1;}
   if(key == 'Z') {near_val *= 0.9; far_val *= 0.9;}
   if(key == 'r') radius *= 2.0;
   if(key == 'R') radius *= 0.5;
   if(key == 'o') theta += dr;
   if(key == 'O') theta -= dr;
